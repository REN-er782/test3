    ...
    def get_dependent_result(self, age_group, income_range):
        ...
        if age_group == "19歳以上23歳未満":
            result_map = {
                ...
                "年収：123万円超〜150万円未満": ("✅ 特定親族特別控除対象", "✅ 被扶養者対象", "✅ 家族手当支給"),
                "年収：150万円超〜160万円未満": ("✅ 特定親族特別控除対象", "❌ 被扶養者対象外", "✅ 家族手当支給"),
                "年収：160万円超〜165万円未満": ("✅ 特定親族特別控除対象", "❌ 被扶養者対象外", "❌ 家族手当不支給"),
                "年収：165万円超〜188万円未満": ("✅ 特定親族特別控除対象　※年末調整にて申請", "❌ 被扶養者対象外", "❌ 家族手当不支給"),
            ...
            result_map = {
                ...
                "年収：123万円超〜130万円未満": ("❌ 控除対象外", "✅ 被扶養者対象", "✅ 家族手当支給"),
                "年収：130万円超〜160万円未満": ("❌ 控除対象外", "❌ 被扶養者対象外", "✅ 家族手当支給"),
                "年収：160万円超": ("❌ 控除対象外", "❌ 被扶養者対象外", "❌ 家族手当不支給")
            }
...
def load_help_content():
    ...
    return {
        ...
        "特定親族特別控除": "19歳以上23歳未満の扶養親族に対する所得控除です。通常の扶養控除より控除額が大きくなります。",
        "被扶養者": "健康保険において、今後1年間の年収が130万円未満の家族が対象となります。この場合の年収は、現時点から1年間になる為、見込み年収で考えてください。該当の家族は出版健康保険組合の健康保険証が使用できます。",
...
def display_guidance():
    ...
    ご家族の収入が変わり、控除や健康保険、手当の対象になる/対象から外れる場合は、
...
def validate_inputs(person_type, **kwargs):
    ...
    if person_type == "spouse":
        if "main_income" not in kwargs or kwargs["main_income"] is None:
            errors.append("納税者本人(あなた)の年収を選択してください")
...
""")
    ...
    with col2:
        ...
        if age_group == "16歳以下":
            ...
            income_child = st.selectbox("", income_options, key="child_income_select")
